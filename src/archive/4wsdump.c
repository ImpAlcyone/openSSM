...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt (argc, argv, "d:f:s:e:")) != -1 )
	{
		switch(opt)
		{
			...
			case 'e': end=get_address(optarg);
				  break;
			default : printf("Usage: 4wsdump [-d device] [-f output_file] [-s start_address] [-e end_address]\n");
				  exit(0);
				  break;
		}
	...
	printf("Dumping 4WS address space from %4.4x to %4.4x\n",start,end);
	...
	{
		if ((rc=ssm_query_4ws(address,&data,1)) != 0)
		{
