...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt (argc, argv, "d:f:s:e:")) != -1 )
	{
		switch(opt)
		{
			...
			case 'e': end=get_address(optarg);
				  break;
			default : printf("Usage: tcudump [-d device] [-f output_file] [-s start_address] [-e end_address]\n");
				  exit(0);
				  break;
		}
	...
	printf("Dumping TCU address space from %4.4x to %4.4x\n",start,end);
	...
	/*--------------------------------*/
	/* Reset the TCU for good measure */
	/*--------------------------------*/
	...
	if ((rc=ssm_romid_tcu(&romid)) != 0)
	{
		printf("ssm_romid_tcu() returned %d\n",rc);
		ssm_close();
		fclose(fh);
		exit(6);
	}
	
	printf("TCU ROM ID is %6.6x\n",romid);
	
	/*-----------------------------------------------*/
	/* Dump the TCU address space to file            */
	/* Takes over 3 hours if you dump the whole lot  */
	/*-----------------------------------------------*/
	
	for (address=start; address<=end; address++)
	{
		if ((rc=ssm_query_tcu(address,&data,1)) != 0)
		{
